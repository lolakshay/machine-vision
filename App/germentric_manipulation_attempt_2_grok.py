...
class PhotoCaptureApp:
    ...
    def show_main_screen(self):
        ...
        self.brightness_scale.pack(anchor="w", pady=5)
        ...
        self.contrast_scale.pack(anchor="w", pady=5)
        ...
        transform_frame = ctk.CTkFrame(self.options_scroll)
        transform_frame.pack(fill="x", pady=15, padx=5)
        ctk.CTkLabel(transform_frame, text="Geometric Transformations", font=ctk.CTkFont(size=18, weight="bold")).pack(anchor="w")

        ctk.CTkLabel(transform_frame, text="Scale Factor (0.1 to 1.5):").pack(anchor="w", pady=5)
        self.scale_slider = ctk.CTkSlider(transform_frame, from_=0.1, to=1.5, command=self.update_scale, width=200)
        self.scale_slider.set(1.0)
        self.scale_slider.pack(anchor="w", pady=5)

        ctk.CTkLabel(transform_frame, text="Interpolation:").pack(anchor="w", pady=5)
        ...
        ctk.CTkLabel(transform_frame, text="Translation X (-100 to 100):").pack(anchor="w", pady=5)
        self.translation_x_slider = ctk.CTkSlider(transform_frame, from_=-100, to=100, command=self.update_translation_x, width=200)
        self.translation_x_slider.pack(anchor="w", pady=5)

        ctk.CTkLabel(transform_frame, text="Translation Y (-100 to 100):").pack(anchor="w", pady=5)
        self.translation_y_slider = ctk.CTkSlider(transform_frame, from_=-100, to=100, command=self.update_translation_y, width=200)
        self.translation_y_slider.pack(anchor="w", pady=5)

        self.affine_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Apply Random Affine", variable=self.affine_var).pack(anchor="w", pady=5)
        self.perspective_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Apply Random Perspective", variable=self.perspective_var).pack(anchor="w", pady=5)

        # Crop coordinates with consistent layout
        crop_coord_frame = ctk.CTkFrame(transform_frame)
        ...
        crop_coord_frame2 = ctk.CTkFrame(transform_frame)
        ...
        ctk.CTkLabel(transform_frame, text="Padding (pixels):").pack(anchor="w", pady=5)
        self.padding_slider = ctk.CTkSlider(transform_frame, from_=0, to=100, command=self.update_padding, width=200)
        self.padding_slider.pack(anchor="w", pady=5)

        self.flip_h_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Flip Horizontal", variable=self.flip_h_var).pack(anchor="w", pady=5)
        self.flip_v_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Flip Vertical", variable=self.flip_v_var).pack(anchor="w", pady=5)
        ...
        ctk.CTkLabel(transform_frame, text="Shear X (-0.5 to 0.5):").pack(anchor="w", pady=5)
        self.shear_x_slider = ctk.CTkSlider(transform_frame, from_=-0.5, to=0.5, command=self.update_shear_x, width=200)
        self.shear_x_slider.pack(anchor="w", pady=5)

        self.warp_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Apply Random Warp", variable=self.warp_var).pack(anchor="w", pady=5)

        self.face_detection_var = ctk.BooleanVar()
        ctk.CTkCheckBox(transform_frame, text="Face Detection", variable=self.face_detection_var).pack(anchor="w", pady=5)

        ctk.CTkLabel(transform_frame, text="Image Label:").pack(anchor="w", pady=5)
        self.label_entry = ctk.CTkEntry(transform_frame, width=200)
